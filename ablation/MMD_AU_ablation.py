...
def generate_kernel_bandwidth(n_bandwidth, X, Y, reg, is_cov, way, encoder):
    ...
    dist = torch_distance(Z, Z, max_size=500, matrix=False)
    for i in range(len(n_bandwidth)):
        if i >= 3 and n_bandwidth[i] > 0:
            ...
            Z_rep = encoder(Z)
            dist = torch_distance(Z_rep, Z_rep, max_size=500, matrix=False)
        kernel = KERNEL_SET[i]
        if way[i] == 'Boost':
...
def TST_MMD_AU(name, N1, rs, check, n_test, n_per, alpha, is_cov_tr, model_au):
    ...
    # model_au.test_bootstrap = MMD_AU.test_bootstrap
    with torch.no_grad():
        ...
        for k in range(n_test):
            ...
            ########################################################
            n_te = N1
            U_te, U_b = model_au.compute_U_stats(
                X_test=X_test, Y_test=Y_test, n_per=n_per)
            if model_au.is_cov_tr:
                L_te = model_au.L_tr  # L_te.size() = (c, c) column vector
            else:
                L_te = torch.inverse(
                    sqrtm(model_au.compute_Sigma(X_test=X_test, Y_test=Y_test)))
            half_te = L_te @ U_te
            half_te_b = L_te @ U_b
            T_obs_no_select = n_te**2 * torch.norm(half_te, p=2)**2
            T_b_no_select = n_te**2 * torch.norm(half_te_b, p=2, dim=0)**2
            ...
            F_te = torch.sign(half_te)
            F_te_b = torch.sign(half_te_b) #[:, 0:2]
            F = F_te == model_au.F_tr
            global count, th, th_s
            count += F.reshape(-1).cpu().numpy()
            F_b = F_te_b == model_au.F_tr
            # count += torch.mean(F_b.to(torch.float32), dim=1).cpu().numpy()
            # print(torch.mean(F_b.to(torch.float32), dim=1))
            T_obs_select = n_te**2 * torch.norm(F * half_te, p=2)**2
            T_b_select = n_te**2 * torch.norm(F_b * half_te_b, p=2, dim=0)**2
            # th.append(torch.sort(T_b_no_select)[0][94].item())
            # th_s.append(torch.sort(T_b_select)[0][94].item())
...
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/MNIST_[1, 1, 1, 1, 1, 1]_True_30_263_1000_128_5e-05.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/MNIST_[1, 1, 1, 1, 1, 1]_True_40_480_1000_128_5e-05.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/MNIST_[1, 1, 1, 1, 1, 1]_True_50_470_1000_128_5e-05.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/MNIST_[1, 1, 1, 1, 1, 1]_True_60_583_1000_128_5e-05.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/MNIST_[1, 1, 1, 1, 1, 1]_True_70_683_1000_128_5e-05.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/BLOB_[3, 0, 3, 0, 0, 0]_True_50_990_1000_100_0.0005.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/BLOB_[3, 0, 3, 0, 0, 0]_True_100_940_1000_100_0.0005.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/BLOB_[3, 0, 3, 0, 0, 0]_True_150_850_1000_100_0.0005.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/BLOB_[3, 0, 3, 0, 0, 0]_True_200_790_1000_100_0.0005.pkl", "rb") as f:
# with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/BLOB_[3, 0, 3, 0, 0, 0]_True_250_750_1000_100_0.0005.pkl", "rb") as f:
with open("/data/gpfs/projects/punim2335/AU_exp/exp_TST/model/BLOB_[3, 0, 3, 0, 0, 0]_True_300_710_1000_100_0.0005.pkl", "rb") as f:
...
# model_au = train_MMD_AU(name, N1, SEED, check, 0, 128, 0.0005, n_bandwidth=(3,3,3,0,0,0), reg=1e-8, way=['Fuse', 'Fuse', 'Agg', 'Fuse', 'Fuse', 'Agg'], is_cov=True, encoder=model)
for seed in range(SEED, SEED+10):
    # model_au = train_MMD_AU(name, N1, seed, check, 0, 128, 0.0005, n_bandwidth=(2,2,2,0,0,0), reg=1e-8, way=['Fuse', 'Fuse', 'Fuse', 'Fuse', 'Fuse', 'Fuse'], is_cov=True, encoder=None)
    count = np.zeros(6)
    th = []
    th_s = []
    H_MMD_AU_select, H_MMD_AU_no_select, P_HSIC_AU_select, P_HSIC_AU_no_select, _, _ = TST_MMD_AU(
        name, N1, seed, check, 500, 5000, 0.05, False, model_au)
    # Comment following 2
    # print(count/100)
