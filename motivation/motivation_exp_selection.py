...
def generate_kernel_bandwidth(n_bandwidth, X, Y, reg, is_cov, way, device):
    ...
    # random.shuffle(k_b_pair)
    # return sorted(k_b_pair[:5])
...
# def pearson_correlation_torch(x, y):
#     """
#     Calculate Pearson correlation coefficient between two PyTorch tensors.
...
def TST_MMD_AU(name, N1, rs, check, n_test, n_per, alpha, N_epoch, batch_size, learning_rate, n_bandwidth, reg=1e-5, way=['Agg', 'Agg', 'Agg', 'Agg'], is_cov=True, is_cov_tr=True, encoder=None):
    ...
    if N1 == 500:
        sigmas[selection_seed] = model_au.compute_Sigma()
...
results = {}
sigmas = {}
for selection_seed in range(100):
    result = []
    for n in [100, 200, 300, 400, 500]:
        H_MMD_AU_select, H_MMD_AU_no_select, P_MMD_AU_select, P_MMD_AU_no_select, _, _ = TST_MMD_AU(
            "BLOB", n, 15, 1, 100, 100, 0.05, 0, 128, 0.001, n_bandwidth=(20, 0, 0, 0), is_cov=True)
        # print(np.sum(H_MMD_AU_select))
        # print(np.sum(H_MMD_AU_no_select))
        result.append(np.sum(H_MMD_AU_no_select))
    results[selection_seed] = (np.mean(result), result)
    print(results)
    break

# import pickle
# with open('results_100.pkl', 'wb') as f:
#     pickle.dump((results, sigmas), f)